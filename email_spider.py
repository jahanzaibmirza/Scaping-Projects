...
class GoogleMapSpiderSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        'FEED_URI': f'input/Google_Maps_With_Emails.csv',
        'RETRY_TIMES': 5,
    ...
    def __init__(self, **kwargs):
        ...
        self.input_file = self.read_csv_file('input/Google_Maps.csv')

    def start_requests(self):
        for record in self.input_file:
            if record.get('Company Website') and record.get('Company Website', '') != '':
    ...
    def parse_website(self, response):
        ...
        all_links = self.get_required_links(response)
        if all_links:
    ...
    def get_emails(response):
        ...
        all_emails.extend(re.findall(r"[-.a-z]+[0-9]+@[^@\s\.]+\.[.a-z]{2,}", str(response.text)))
        all_emails.extend(re.findall(r"[-.a-z]+@[^@\s\.]+\.[.a-z]{2,}", str(response.text)))
